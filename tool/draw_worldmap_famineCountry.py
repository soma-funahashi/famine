###########################################################
#to          : draw a map of famine database in each year
...
### input data
df=pd.read_csv("../out/famineCountry.csv")
df=df.fillna(0)
iso3=df["ISO3"]
...
year=s[1]
data=df[str(year)]
fn_out="famineCountry_"+str(year)+".png"
...
ax.set_title("Estimated Famine Country in "+str(year))
...
#plt.show()
plt.savefig("../fig/famineCountry/"+fn_out,bbox_inches="tight")
