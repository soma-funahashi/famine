...
gdp_st = pd.DataFrame(scipy.stats.zscore(gdp, axis = None), index=gdp.index, columns=gdp.columns)
upp_st = pd.DataFrame(scipy.stats.zscore(upp, axis = None), index=upp.index, columns=upp.columns)
cor_st = pd.DataFrame(scipy.stats.zscore(cor, axis = None), index=cor.index, columns=cor.columns)
gin_st = pd.DataFrame(scipy.stats.zscore(gin, axis = None), index=gin.index, columns=gin.columns)
